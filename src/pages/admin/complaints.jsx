...
const Complaints = () => {
  ...
  const fetchComplaints = async () => {
    try {
      const response = await fetch('https://ecommerce-backend-owgx.onrender.com/complaints/get-complaints');
      const data = await response.json();
  ...
  const handleStatusChange = async (complaintId, newStatus) => {
    try {
      const response = await fetch('https://ecommerce-backend-owgx.onrender.com/update-complaint-status', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  return (
    <div className="flex bg-pink-50 min-h-screen">
      <Helmet>
        <title>Complaints | Admin | Mytalorzone</title>
      </Helmet>
      <Sidebar />
